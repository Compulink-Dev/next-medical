                    ...
                    {clinics.map((clinic) => (
                        <Dialog key={clinic.id}>
                            <DialogTrigger asChild>
                                <div className="bg-dark-400 shadow-md rounded-lg p-4 cursor-pointer" onClick={() => setActiveClinic(clinic)}>
                            ...
                            <DialogContent className="sm:max-w-md">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>{activeClinic?.description}</DialogDescription>
                                </DialogHeader>
                                <Image width={80} height={80} className="w-full h-48 object-cover rounded-lg" src={activeClinic?.image || ''} alt={activeClinic?.name || ''} />
                                <DialogFooter>
                                    <Button variant={'outline'} onClick={() => setActiveClinic(null)}>Close</Button>
                                </DialogFooter>
                            </DialogContent>
                        </Dialog>
                    ))}
