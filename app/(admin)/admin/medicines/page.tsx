...
import { getAllMedicines, getRecentMedicineList } from '@/lib/actions/medicine.action';
...
async function Medicines() {
    ...
    console.log('medicines:', medicalData);

    // Calculate statistics
    const lowStockCount = medicalData?.documents?.filter(med => med.status === 'Low Stock').length || 0;
    const outOfStockCount = medicalData?.documents?.filter(med => parseInt(med.stock) === 0).length || 0;
    const expiredCount = medicalData?.documents?.filter(med => new Date(med.expiryDate) < new Date()).length || 0;
    return (
                        ...
                        >
                            {
                                medicines?.map((clinic, index) => (
                                    <SelectItem key={index} value={clinic.category}>{clinic.category}</SelectItem>
                                ))
                            }
                        </InputSelect>
                        <Button>
